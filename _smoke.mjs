...
async function post(path, body, headers = {}) {
  const r = await fetch(`${API}${path}`, {
    ...
    headers: {
      ...
      Origin: 'https://www.interdependent.studio',
      ...headers,
    },
...
try {
  ...
  {
    const noEmail = await post('/readers/register/begin', {
      firstName: 'Smoke',
      lastName: `Test${SUFFIX}`,
    });
    ok(
      noEmail.status === 400 && noEmail.body.code === 'email_required',
      `no email → 400 email_required (got ${noEmail.status}/${noEmail.body.code})`,
    );
    const withEmail = await post('/readers/register/begin', {
      firstName: 'Smoke',
      lastName: `Test${SUFFIX}`,
      email,
    });
    ok(
      withEmail.status === 200 && withEmail.body.challengeId && withEmail.body.options,
      `with email → 200 + challenge/options (got ${withEmail.status})`,
    );
  }
  ...
  console.log('\n2. seed a throwaway reader WITH email (service_role)');
  {
    const { error } = await db
      .from('readers')
      .insert({ id: readerId, handle, display_name: `Smoke Test${SUFFIX}`, email });
  ...
  console.log('\n3. recover/request — matching handle+email creates a token');
  {
    ...
    ok(
      data && data[0] && !data[0].used_at && new Date(data[0].expires_at) > new Date(),
  ...
  {
    const before =
      (await db.from('reader_recovery_tokens').select('id').eq('reader_id', readerId)).data
        ?.length || 0;
  ...
  console.log('\n5. recover/begin — valid token returns registration options');
  {
    ...
    await db
      ...
      .eq('reader_id', readerId)
      .is('used_at', null);
    await db.from('reader_recovery_tokens').insert({
      reader_id: readerId,
    ...
    ok(
      good.status === 200 &&
        good.body.challengeId &&
        good.body.options &&
        good.body.handle === handle,
      `valid token → 200 + options + handle (got ${good.status})`,
    );

    const bad = await post('/readers/recover/begin', {
      readerId,
      token: randomBytes(32).toString('hex'),
    });
    ok(
      bad.status === 400 && bad.body.code === 'recovery_invalid',
      `bogus token → 400 recovery_invalid (got ${bad.status}/${bad.body.code})`,
    );
  }
  ...
  console.log('\n6. auth gating on protected endpoints (no action token → 401)');
  {
    ...
    ok(
      add.status === 401 && add.body.code === 'action_token_missing',
      `add-device/begin → 401 (got ${add.status}/${add.body.code})`,
    );
