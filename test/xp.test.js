...
const SCRIPTS = [
  { id: 's1', page_count: 100 },
  { id: 's2', page_count: 100 },
];
...
test('early spot is scarce: only the first few champions, and only if the crowd follows', () => {
  ...
  const champions = champers.map((r, i) => ({
    reader_id: r.id,
    script_id: 's1',
    added_at: `2026-0${i + 1}-01T00:00:00Z`,
  }));
  const stats = aggregateReaderStats({
    readers: champers,
    events,
...
test('the event perk fails CLOSED when no featured script is resolved', () => {
  ...
  const feedback = [
    { reader_id: 'r1', script_id: 's1', dimensions: { a: 5 }, text: 'x'.repeat(200) },
  ];
  const stats = aggregateReaderStats({
    readers: [READER],
    events,
