...
function aggregate(events, scripts, readers, champions) {
  ...
  scripts.forEach((s) => {
    title[s.id] = s.title;
    pages[s.id] = s.page_count;
  });
  const rname = {};
  readers.forEach((r) => {
    rname[r.id] = r.display_name || r.handle;
  });
  ...
  for (const e of events) {
    ...
    if (e.reader_id) {
      const a =
        readerAct[e.reader_id] || (readerAct[e.reader_id] = { scripts: new Set(), opens: 0 });
      if (e.script_id) a.scripts.add(e.script_id);
      if (OPEN.has(e.event_type)) a.opens++;
    }
    const day = (e.ts || '').slice(0, 10);
    if (day) {
      const d = byDay[day] || (byDay[day] = { reads: 0, champions: 0, recommends: 0 });
      if (OPEN.has(e.event_type)) d.reads++;
      if (e.event_type === 'recommend_open') d.recommends++;
    }
  }
  ...
  const recCtx = {};
  events.forEach((e) => {
    if ((e.recommender || e.source === 'recommend') && e.script_id) {
      const k = `${e.session_id}::${e.script_id}`;
      recCtx[k] = recCtx[k] || { opened: false, recommender: e.recommender || null };
      if (e.event_type === 'recommend_open' || OPEN.has(e.event_type)) recCtx[k].opened = true;
    }
  });
  Object.values(reads).forEach((r) => {
    ...
    if (recCtx[k]) {
      recCtx[k].depth = r.depth;
      recCtx[k].seconds = r.seconds;
      recCtx[k].finished = q.finished;
      recCtx[k].engaged = q.engaged;
    }
  });
  Object.entries(recCtx).forEach(([k, c]) => {
    rec[k] = c;
  });
  ...
  champions.forEach((c) => {
    const day = (c.added_at || '').slice(0, 10);
    if (day && byDay[day]) byDay[day].champions++;
  });
  const championList = champions
    .map((c) => ({
      script_id: c.script_id,
      title: title[c.script_id] || '(unknown)',
      reader: rname[c.reader_id] || 'A reader',
      addedAt: c.added_at,
    }))
  ...
  const scriptIds = new Set([
    ...Object.keys(per),
    ...Object.keys(champByScript),
    ...Object.keys(perRead),
  ]);
  const perScript = [...scriptIds]
    .map((scr) => {
      ...
      return {
        script_id: scr,
        title: title[scr] || '(unknown)',
        opens: p.opens,
        uniqueReaders: p.sessions.size,
        reachedAvg: depthAvg,
        avgSeconds: timeAvg,
        engaged: rd.filter((r) => r.engaged).length,
        finished: rd.filter((r) => r.finished).length,
        champions: (champByScript[scr] || []).length,
        recommends: p.recommends,
      };
    })
    .sort(
      (a, b) => b.opens - a.opens || b.finished - a.finished || b.uniqueReaders - a.uniqueReaders,
    );
  ...
  const recommendFunnel = {
    ...
    read25: rs.filter((r) => (r.depth || 0) >= 25).length,
    read75: rs.filter((r) => (r.depth || 0) >= 75).length,
    engaged: rs.filter((r) => r.engaged).length,
    finished: rs.filter((r) => r.finished).length,
  };
  const byRecommender = {};
  rs.forEach((r) => {
    const n = r.recommender || '—';
    const a =
      byRecommender[n] ||
      (byRecommender[n] = { recommender: n, sent: 0, opened: 0, engaged: 0, finished: 0 });
    a.sent++;
    if (r.opened) a.opened++;
    if (r.engaged) a.engaged++;
  ...
  const readerList = readers
    ...
    .filter((r) => r.championed > 0 || r.scriptsRead > 0)
    .sort((a, b) => b.championed - a.championed || b.scriptsRead - a.scriptsRead);

  return {
    totals: {
      scriptViews: counts.script_view,
      readerOpens: counts.reader_open,
      quickPreviews: counts.quick_preview,
      finishedReads: completions,
      downloads: counts.download,
      newTabs: counts.new_tab,
      champions: champions.length,
      recommendOpens: counts.recommend_open,
...
function scriptDetail(id, events, scripts, readers, champions) {
  ...
  const title = sc.title || '(unknown)';
  const pageCount = sc.page_count;
  const rname = {};
  readers.forEach((r) => {
    rname[r.id] = r.display_name || r.handle;
  });
      ...
      const verdict = q.finished
        ...
        : q.engaged
          ...
          : s.depth >= 25
            ...
            : 'opened';
      return {
        name: s.reader || 'Anonymous reader',
        depth: s.depth,
        seconds: s.seconds,
        verdict,
  ...
  readerRows.forEach((s) => {
    if (s.recommender) {
      ...
      a.sent++;
      if (s.verdict !== 'opened') a.opened++;
      if (s.verdict === 'engaged' || s.verdict === 'finished') a.engaged++;
      if (s.verdict === 'finished') a.finished++;
    }
  });
  ...
  return {
    ...
    summary: {
      ...
      avgDepth: readerRows.length
        ? Math.round(readerRows.reduce((a, s) => a + s.depth, 0) / readerRows.length)
        : 0,
      avgSeconds: readerRows.length
        ? Math.round(readerRows.reduce((a, s) => a + s.seconds, 0) / readerRows.length)
        : 0,
    },
    readers: readerRows,
    champions: champs,
    recommenders: Object.values(recBy),
    timeline,
  };
}
...
router.get('/script/:id', async (req, res, next) => {
  try {
    const [events, scripts, readers, champions] = await Promise.all([
      listReadEvents({ scriptId: req.params.id }),
      getScriptTitles(),
      getReaders(),
      getChampions(),
    ]);
