...
if (!markApplied && markNames.length)
  fail(`unexpected arguments: ${markNames.join(' ')} (names are only valid with --mark-applied)`);
...
function psqlBackend(url) {
  ...
  const run = (args, input) => {
    ...
    if (res.error?.code === 'ENOENT')
      fail('psql not found on PATH (needed for the DATABASE_URL backend)');
  ...
  return {
    ...
    exec: async (sql) => {
      run(['--single-transaction', '-f', '-'], sql);
    },
    query: async (sql) =>
      JSON.parse(
        run(['-A', '-t', '-c', `SELECT COALESCE(json_agg(t), '[]'::json) FROM (${sql}) t`]),
      ),
  };
}
...
if (ledger_exists) {
  for (const row of await backend.query(
...
for (const [name, row] of applied) {
  ...
  else if (checksumOf(name) !== row.checksum)
    console.warn(`warn: ${name} changed on disk after being applied (checksum mismatch)`);
}
...
if (status) {
  ...
  if (!ledger_exists)
    console.log('(no schema_migrations table yet — nothing has been applied through the runner)');
  process.exit(0);
}
...
if (markApplied) {
  ...
  for (const name of targets) {
    ...
    if (dryRun) {
      console.log(`would mark applied  ${name}`);
      continue;
    }
...
if (dryRun)
  console.log(`dry run — ${pending.length} migration(s) would be applied, no changes made`);
