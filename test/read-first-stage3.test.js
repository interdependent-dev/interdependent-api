...
test('earlyOpinionSpot: first FINISHED opinion on a crowd-validated script earns it; farming is blocked', async () => {
  ...
  const readers = [
    { id: 'A', handle: 'a', display_name: 'A' },
    { id: 'B', handle: 'b', display_name: 'B' },
  ];
  const scripts = [
    { id: 'S', page_count: 10 },
    { id: 'T', page_count: 10 },
  ...
  const events = [
    { event_type: 'read_progress', reader_id: 'A', script_id: 'S', depth_pct: 95, seconds: 220 },
  ];
  const feedback = [
  ...
  assert.equal(
    stats.A.earlyOpinions,
    1,
    'A: first + finished + crowd on S (T excluded: no crowd / unfinished)',
  );
