...
router.get('/:scriptId', requireAuth, optionalReader, async (req, res, next) => {
  try {
    ...
    let messages = [];
    try {
      messages = await getScriptMessages(scriptId, req.reader.id);
    } catch {
      messages = [];
    } // chat tables not migrated yet → empty, not an error
...
router.post('/:scriptId', requireActionToken, async (req, res, next) => {
  try {
    ...
    if (!(await canChat(req.reader, scriptId))) {
      return next(
        new AppError(
          'Only readers who championed this script can join the chat',
          403,
          'not_a_champion',
        ),
      );
    ...
    let msg;
    try {
      msg = await insertMessage({ scriptId, readerId: req.reader.id, parentId, body });
    } catch {
      return next(new AppError('The chat is not available right now', 503, 'chat_unavailable'));
    } // fail-open (e.g. tables not migrated); never leak the raw DB error
    res.status(201).json(msg);
  } catch (err) {
    next(err instanceof AppError ? err : new AppError('Could not post message', 500));
  }
});
...
router.post('/:messageId/endorse', requireActionToken, async (req, res, next) => {
  try {
    ...
    if (meta.reader_id === req.reader.id)
      return next(new AppError('You cannot endorse your own message', 400, 'self_endorse'));
    ...
    try {
      await endorseMessage({ messageId, endorserId: req.reader.id });
    } catch {
      return next(new AppError('The chat is not available right now', 503, 'chat_unavailable'));
    }
    res.json({ ok: true });
  } catch (err) {
    next(err instanceof AppError ? err : new AppError('Could not endorse', 500));
  }
});
