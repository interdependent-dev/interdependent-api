...
test('chat XP: endorsements-received + conversations-sparked, self-signals excluded', async () => {
  ...
  const readers = [{ id: 'A', handle: 'a' }, { id: 'B', handle: 'b' }];
  const chat = {
    messages: [
  ...
  const stats = aggregateReaderStats({ readers, events: [], champions: [], feedback: [], scripts: [], chat });
...
test('chat XP is zero when there is no chat data (fail-open default)', async () => {
  const { aggregateReaderStats } = await import('../src/lib/xpAggregate.js');
  const stats = aggregateReaderStats({ readers: [{ id: 'A', handle: 'a' }], events: [], champions: [], feedback: [], scripts: [] });
