...
if (!APPLY) {
  console.log('\nDRY RUN — backups written, nothing deleted. Re-run with APPLY=1.');
  process.exit(0);
}
...
if (dErr) {
  console.log('Row delete FAILED:', dErr.message);
  process.exit(1);
}

// Verify.
const { count: rejLeft } = await db
  .from('scripts')
  .select('*', { count: 'exact', head: true })
  .eq('status', 'rejected');
const { count: evLeft } = await db
  .from('read_events')
  .select('*', { count: 'exact', head: true })
  .in('script_id', ids);
console.log(
  `✓ Deleted ${ids.length} rows. Rejected remaining: ${rejLeft}. Orphan read_events for those ids: ${evLeft}.`,
);
