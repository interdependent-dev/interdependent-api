...
for (const r of readers) {
  ...
  const { data: lb } = await db
    ...
    .select('script_id,position')
    .eq('reader_id', r.id)
    .order('position');
  console.log(`\n• ${r.display_name}  [${r.handle}]  id=${r.id}  created=${r.created_at}`);
  console.log(
    `  read_events=${ev}  feedback=${fb}  passkeys=${creds?.length || 0}  leaderboard=${lb?.length || 0}`,
  );
  for (const c of creds || [])
    console.log(
      `    cred ${c.credential_id.slice(0, 12)}… ${c.device_type} backedUp=${c.backed_up} created=${c.created_at} lastUsed=${c.last_used_at ?? 'never'}`,
    );
}
