...
// 1. readers.email column exists?
{
  ...
  if (error) {
    console.log('✗ readers.email  MISSING —', error.message);
    ok = false;
...
// 2. reader_recovery_tokens table exists?
{
  const { error } = await db
    ...
    .limit(1);
  if (error) {
    console.log('✗ reader_recovery_tokens MISSING —', error.message);
    ok = false;
...
if (ok) {
  ...
  if (error) console.log('? readers list error:', error.message);
    ...
    for (const r of data)
      console.log(
        `  ${r.email ? '✓' : '—'} ${r.handle.padEnd(22)} ${r.email || '(no email — recovery unavailable)'}`,
      );
  }
}
